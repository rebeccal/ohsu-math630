...
library(multcomp)
mcp <- glht(heroaov,linfct=mcp(hero="Tukey")) # "Tukey" sets up the pairwise comparisons (computes the estimate and SE, but not the correction. see "test" below) 
#summary(mcp) # Tukeys with single-step adjusted p-value
#summary(mcp, test=univariate()) #like a set of paired-ts (pooled sd 2 groups), but with pooled sd from all groups) - same as pairwise.t.test() above
