...
We have some groups with pretty small *n*'s. Specifically, there are only *n*=4 American Indians, *n*=26 Multi-race, *n*=3 Native Hawaiians, and *n*=2 "Other". We have two choices: lump (with another category) or dump (from the entire analysis). For simplicity, I am going to rename "White not Hispanic" as "WNH", leave "Hispanic" as is, and lump all other designations into an "Other" category.
...
library(ggmosaic)
p <- ggplot(data = ca2) +
   geom_mosaic(aes(x = product(agegroup,eth3), fill=agegroup), na.rm=TRUE) +
  labs( x = "Ethnicity", y = "AgeGroup", title = "Group Counts") +
   theme(legend.position="none") 
  
p + geom_text(data = ggplot_build(p)$data[[1]], aes(x = (xmin+xmax)/2, y = (ymin+ymax)/2, label=.wt), size = 3)
...
```{r include=FALSE}
agebox2 <- ggplot(data=ca2, aes(agegroup, exp)) +
  geom_boxplot(aes(fill = eth3)) + 
  #guides(fill=FALSE) +
  labs(x = "Age Group", y = "Mean Expenditures") +
  theme_bw() 
agebox2
```
