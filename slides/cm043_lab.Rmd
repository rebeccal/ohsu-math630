...
## Random Sampling
Remember, our observed mean is `r round(mean_diff, 2)`. So, this new resampled mean difference is `r round(sn1_diff, 2)`, calculated as if it didn't matter if the real `"stars"` were `"stars"`, is much smaller. Now, this was just one possible resample. How many possible ways are there to choose 16 observations from 40? 
...
That is, there are `r choose(40, 16)` possible permutations:
...
Remember that the observed difference in means was `r round(pull(mean_diff),2)`. We have shaded red all values at or above that value. By giving `obs_stat = mean_diff`, a vertical darker line is also shown at `r round(pull(mean_diff),2)`. 
