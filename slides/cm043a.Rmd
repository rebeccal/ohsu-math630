---
title: "CM 4.3 -Resampling-based hypothesis testing"
...
- This lab is based on the assigned [ModernDive Chapter 9 and Appendix B readings](https://moderndive.org). Please open and follow closely!
...
In our [CM 4.2 code-along](cm042-code_along.html), we used data to estimate the average height of males in the US. The sample data was from the [CDC's 2016 Behavioral Risk Factor Surveillance System (BRFSS)](https://www.cdc.gov/brfss/annual_data/annual_2016.html):
...
skim(male_heights) # Because skim on windows can be unattractive
...
Next, we quantified the *precision* of our estimate of the mean using bootstrapping to simulate the sampling distribution of the sample mean. [Recall](https://moderndive.com/7-sampling.html#student-shovels) that sampling distributions are a specific kind of distribution: distributions of *point estimates/sample statistics* based on samples of size $n$ used to estimate an unknown *population parameter*. Here is the code  used:

```{r sampling_dist_1000, include=FALSE, eval=FALSE}
set.seed(1701)
...
```{r, sampling_dist_500, cache=TRUE}
set.seed(1701)
mh_means500 <- male_heights %>% 
  specify(response = HTM4) %>% 
  generate(reps = 500, type = "bootstrap") %>% 
  calculate(stat = "mean")

mh_means <- mh_means500 # Or whatever number of reps your memory allowed
```
...
mh_means %>% 
  visualize() +
  shade_confidence_interval(percentile_ci1)
```

In fact, we could use `dplyr` to count how many bootstrapped means fall outside of the 95% confidence interval (the green shaded region bounded by the two green lines). 

Counting the number of bootstrapped means shows that we have $13/500 = .026$ samples below the lower bound (~2.5%) CI (shaded below in red). #Note - this comes out to an even 2.5% if you use 1000 reps ;)
...
mh_means %>% 
  visualize() +
  shade_p_value(pull(percentile_ci1[1]), direction = "less")
```

We also count $13/500 = .026$ samples above the upper bound (~97.5%) CI.
...
mh_means %>% 
  visualize() + 
  shade_p_value(pull(percentile_ci1[2]), direction = "greater")

```

So, our original sample mean was `r round(x_bar1,2)`. We used bootstrapping to quantify the precision of that sample estimate. Finally, we used `infer::get_ci(type = "percentile")` to calculate the range of "stats" (here, sample means) that encompasses 95% of the bootstrapped samples- > 5% of our bootstrapped samples produce means that fall outside of this range.

# Key Questions

See [Chapter 7 in ModernDive](https://moderndive.com/7-sampling.html#concepts-related-to-sampling).
...
1. What is the population parameter of interest?
    - Hint: https://moderndive.com/8-confidence-intervals.html
...
> Assuming that this sample is representative of all males in the US, if we were to collect 500 random samples using the exact same sampling method, in 95% of *those* samples (i.e., 475 out of 500), the average height for males in the US would be between ---- and ---- centimeters.
...
Follow along in [ModernDive, Appendix B.2.4](http://moderndive.com/b-appendixb).

Using the "OKCupid" sample data described above, we'll use bootstrapping to evaluate the following competing hypotheses:

## In words 

- Null hypothesis: The mean height for all US men is equal to 178.06 cm (as we found above)
...
__Think:__ if the population mean *were* 178.06, how surprised would I be to get a sample mean as or more extreme than the one I got?



## The bootstrap approach

First step is to calculate the sample mean height using yourOkCupid sample.
...
In order to see if the observed sample mean of `r cupid_bar %>% pull() %>% round(2)` is statistically greater than $\mu_0 = 178.06$, we need to account for the sample size.  We also need to determine a process that replicates how the sample of size `r nrow(cupid)` was selected.  
...
1. Sample with replacement from our OKCupid sample of `r nrow(cupid)` men and repeat this process 1,000 times (try `set.seed(2018)`), 
...
null_cupid_1000 %>% 
  visualize() +
  shade_p_value(cupid_bar, direction = "greater")
...
Follow along in [ModernDive, Appendix B.2.4](https://moderndive.com/B-appendixB.html#non-traditional-methods).

1. Create a confidence interval for the unknown population parameter $\mu$ using our "OKCupid" sample data and *bootstrapping*. 
...
__Question:__ What might explain our conflicting results?
...
cupid_means %>% 
  visualize() +
  shade_confidence_interval(percentile_ci2) +
  shade_p_value(x_bar1, direction = NULL)
...
#The below description is for "paired" data, and a bit of a mess.
...
Follow along with the [ModernDive Chapter 9](https://moderndive.com/9-hypothesis-testing.html) and [ModernDive Appendix B.5](https://moderndive.com/B-appendixB.html#two-means-independent-samples).
...
FYI - The dataset is one the course web site, under [Reference Plus/data](https://ohsu-math630-fall-2019.netlify.com/reference.html):
