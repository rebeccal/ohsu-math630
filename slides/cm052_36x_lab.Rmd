...
```{r}
#RNGkind(sample.kind = "Rounding") #This allows the set.seed in 3.6.x to produce the same output as 3.5.x
set.seed(1990)
...
(anova_mini_lm <- anova(mini_lm))
```

```{r glht_mini,  eval = FALSE}
#library(multcomp) #Avoiding loading multcomp as it masks dplyr::select
multcomp::glht(mini_lm)
summary(multcomp::glht(mini_lm, linfct = multcomp::mcp(age_groups = "Tukey")))
```
