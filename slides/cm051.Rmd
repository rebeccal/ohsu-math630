---
title: "Math 530/630 CM 5.1"
subtitle: "ANOVA: Comparing More Than Two Sample Means"
#author: "Alison Hill"
output:
...
???
* Ex 1: Beethoven,Stones, None
* Ex 2: 1) Beethoven, Stones 2) Male, Female

t-test: special case. One predictor, two levels, one outcome
...
* The first subscript refers to the row that the particular value is in; *i* = 1, 2, 3, ...,*n*
...
???
j = groups/conditions
i = observations (NOT same subject/person)

. = means, summary across
.. = Grand Mean

Null Hypothesis: j's don't matter!!
...
???
* Observed - expected if the groups didn't matter
* $\bar{y}$ instead of $\hat{y}$ = group means instead of fitted values
---
...
???
* Group agnostic - no j's
* Deviation of the observed values from the grand mean
* Dots vs the dotted line
...
* Generally: $df_{total} = kn-1$ (k groups and n per group)
...
Re-write this using bar/dot notation, and acknowledging that our total $N = k\times{n}$:
...
???
* Total = just the variance in y (outcome)
* Here = how much do the group means vary from each other
  * ? n*var(group means - columnwise)
* Many names - I like between
...
For more than one group, we pool our estimates across the *j* groups to be a more precise estimate of the population variance, $\sigma^2$:
...
???
*               
* $MS_{total}$  Values      vs Grand Means
* $Ms_{model}$  Group Means vs Grand Means
* $Ms_{Resid}$  Values      vs Group Means
...
* D = $\frac{(\sum_{j = 1}^kB_j)^2}{kn}$ (here, D = 507)
...
```{r eval = FALSE}
superhero <- read.delim(file.choose(), header = T, sep=",")
```
...
```{r include = FALSE}
superhero <- read_csv(here::here("data", "Superhero.csv"))
...
<!-- ## Next lecture: where does the difference lie? -->

<!-- ```{r echo = FALSE, fig.width=2.75, fig.height=1.75, dpi = 300} -->
<!-- library(ggplot2) -->
