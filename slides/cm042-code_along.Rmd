...
Let's do 3 resamplings and compare them.
...
__Questions:__ In `mh_resample1`:
...
__Question:__ How would you recreate these two calculations by "hand" with the help of R? Do so.
...
```
__Question:__ What did we gain by using a subset? What did we lose? What if we used an even smaller sample?



```{r misc, include=FALSE}
# Using the last distribution to determine the CI "by hand"

# Percentile method
sorted_means <- mh_means_samp %>%
  arrange(stat)
sorted_means$stat[26] # Because there's 25 points below 26, 
sorted_means$stat[975] # and 25 points above 975

# SE method
# This works because we know that 95% of the probability is between qnorm(0.25) and qnorm(.975) SEs of the mean.
# In theory, we shouldn't be using qnorm, but qt instead. However, at n=2000, they are functionally similar.
mh_means_samp %>% 
  summarize(mean_of_means = mean(stat),
            sd_of_means = sd(stat),
            ci_lower = mean_of_means + sd_of_means*qnorm(.025),
            ci_upper = mean_of_means + sd_of_means*qnorm(.975))
```
