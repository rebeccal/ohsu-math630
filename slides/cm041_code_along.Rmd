...
Up until now, we've exhaustively enumerated all possible combinations before computing our sample means. But now we want to do 10 rolls!! 

1. How many combinations would there be, if we enumerated all possible combinations? (HINT: don't really do it!) 
1. Can we instead just draw random samples? Why or why not?
1. What is the sampling distribution of the sample mean for a uniformly distributed variable? Use the following code to simulate 10 experiments where a die is rolled 10 times and we take the mean pips for each experiment. 
...
for (i in 1:10) {
  y <- runif(10, min = 1, max = 6) # draw 10 random samples from the unif dist
...
2. Use the code above to run this simulation for 100,000 (or 10,000) experiments, and make a plot of the sampling distribution of the sample means. (HINT: You may need to erase some objects before runnning 100,000. Use 'rm(object_name)'.) What do you see?

```{r include=FALSE}
set.seed(12345)
unif_means_ten <- numeric(10) # space for big results (vector of Os)
for (i in 1:10000){
  y <- runif(10, min = 1, max = 6) # draw random sample with 10 observations each
...
ggplot(my_sim2, aes(x = sample_mean)) +
  ...
  stat_function(fun = dnorm, args = list(mean = mm2, sd = sem2), color = "red") 
```
