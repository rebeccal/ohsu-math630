---
title: "MATH 530/630"
subtitle: "Integrative Lab 0 - Introduce Yourself!"
output:
  html_document:
    toc: TRUE
    toc_float: TRUE
...
> Read all the way through step 6, and note that there is a file that needs to be turned in to Sakai! 
...
* They may be carefully curated by CRAN (which involves a thorough submission and review process), and thus are easy to install using:
```{r eval = FALSE} 
...
If you have done the above and produced sane-looking output, you are ready for the next bit. Use the code below to create a new data frame called `rebecca`.

```{r manipulate_name_data}
rebecca <- babynames %>%
  filter(name == "Rebecca" | name == "Rebekah") %>% 
  filter(sex == "F")
```

* The first bit makes a new dataset called `rebecca` that is a copy of the `babynames` dataset- the `%>%` tells you we are doing some other stuff to it later.

* The second bit `filters` our `babynames` to only keep rows where the `name` is either Rebecca or Rebekah (read `|` as _"or"_.) 
...
```{r plot_name_data}
plot <- ggplot(rebecca, aes(x = year, 
                           y = prop,  
                           group = name, 
                           color = name)) + 
  geom_line()  
...
Edit the code above to create a new dataset. Pick 2 names to compare how popular they each are (these could be different spellings of your own name, like I did, but you can choose any 2 names that are present in the dataset). Make the new plot, changing the name of the first argument `rebecca` in `ggplot()` to the name of your new dataset.
...
```{r eval = F}
ggsave("rebecca_lunsford.pdf", plot) # please make the filename unique!
```

# Grading

There is no self-assessment component to this iLab, because you'll need these tools to complete the future assignments. Hopefully, this should be an easy 20 points!
