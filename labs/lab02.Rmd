---
title: "MATH 530/630"
subtitle: Integrative Lab 2 - Linear Regression
output:
  html_document:
    toc: yes
    toc_float: yes
...
# leverage
k <- 4 #There are 4 predictors/coefficients, sex, age, sex X age, and z-score
# k <- sum(d_diag$.hat)
mean_hat <- (k + 1)/nrow(vitd) # Or, mean(d_diag$.hat)
d_diag %>% 
  select(id, .hat) %>%
...
```{r include = FALSE}
n <- nrow(vitd)
k <- 4 # predictors (not including intercept) There are 4 predictors, sex, age, sex X age, and z-score 
d <- 4/(n - k - 1)
d
