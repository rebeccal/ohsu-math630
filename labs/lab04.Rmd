---
title: "MATH 530/630"
subtitle: Integrative Lab 4 - ANOVA
output:
  html_document:
    highlight: pygments
    smart: no
...
- Also this slide deck on [the general linear model](../slides/cm046.html) may be helpful.
- Also this slide deck on [comparing multiple sample means](../slides/cm051.html).
- This slide deck on [post-hoc tests and p-value adjustment](../slides/cm053.pdf).
- This [code-along](../slides/cm053-code_along.R).  
...
```{r load_packages, include = TRUE, message=FALSE}
library(tidyverse)
library(moderndive)
library(broom)
library(infer)
...
Explore the `patient_type`, `season`, and `vitamin_d` variables. Recall that a [new exploratory data analysis](http://moderndive.com/6-regression.html#model1EDA) involves three things:
...
Let's start with two predictors in our ANOVA model, using addition first (i.e., `+`). No interaction term is estimated, meaning we are not allowing for estimated non-parallel lines. 
...
The majority of the time, if you are interested in doing an ANOVA, it is unlikely that you want output where the order of the predictors in your model formula matters. This type of output is called sequential sums of squares, also known ANOVA with Type I sum of squares. Here is [more information](https://mcfromnz.wordpress.com/2011/03/02/anova-type-iiiiii-ss-explained/) on the 3 types of sums of squares in the context of an ANOVA: 
...
More [reading on this topic here](https://stats.stackexchange.com/a/20455).
...
* Often with ANOVA, you want to plot the *adjusted means*, which are the fitted/predicted values from the ANOVA model (just as were created with linear regression models we have worked with before). Use this code to use the `phia` package to calculate the fitted group means and plot them. Read the [vignette](https://cran.r-project.org/web/packages/phia/vignettes/phia.pdf). Write a few sentences here about the model output and this figure, making sure to note *why* the plots that contain two colors show lines that are parallel ([hint](https://moderndive.com/6-multiple-regression.html#model4table)). 
...
# Two-way ANOVA with Type II SS - Interaction
...
* Use the `phia` package to plot the fitted means from this model. What do you see? What is different now? ([hint](http://moderndive.com/6-multiple-regression.html#model4interactiontable)). 
