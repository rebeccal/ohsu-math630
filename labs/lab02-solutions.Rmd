---
title: "MATH 530/630"
subtitle: 'Integrative Lab 2 - Linear Regression: Self Assessment'
output:
  pdf_document:
    toc: yes
  html_document:
...
Conduct a thorough EDA of the four variables defined above in the vitamin D dataset. Recall that a [new exploratory data analysis](http://moderndive.com/5-regression.html#model1EDA) involves three things:
...
- Parallel slopes example [here](http://moderndive.com/6-multiple-regression.html#model4table)
- Interaction model [here](http://moderndive.com/6-multiple-regression.html#model4interactiontable)
...
- Parallel slopes example [here](http://moderndive.com/6-multiple-regression.html#model3residuals)
- Interaction model [here](http://moderndive.com/6-multiple-regression.html#model4residuals)
...
# leverage
k <- 4 # Number of predictors/coeeficients excluding the intercept, i.e. age, sex, age*sex, z_score
# OR,  k <- sum(d_diag$.hat)-1 # The sum of the .hat scores is the number of coefficients, including the intercept

mean_hat <- (k + 1)/nrow(d_diag) # Or, mean(d_diag$.hat)
d_diag %>% 
  select(id, .hat) %>%
...
```{r include = TRUE}
n <- nrow(d_diag)
k <- 4 # predictors (not including intercept)
d <- 4/(n - k - 1)
d
