---
title: "MATH 530/630"
subtitle: Integrative Lab 1 - Exploratory Data Analysis
output:
  html_document:
    toc: yes
    toc_float: yes
...
knitr::opts_chunk$set(warning = FALSE, message = FALSE, comment = NA, dpi = 300)
```
...
You will start with the `reprohealth` dataset, which is distributed as an R package from [Alison Presmanes Hill's github ](https://github.com/apreshill/reprohealth). 
...
- Which are categorical variables (numeric or character variables that have a fixed and known set of possible values; aka factor variables)?
...
- Read the data file into your EDA .Rmd document using the [`readxl` package](https://readxl.tidyverse.org) (remember, it must be installed first). Use the `here` package, with the `here` function, to build up the relative file path. Below is example code using `here::here`.  
...
```{r}
# Reading in an example dataset using 'here'
...
alcohol <- read_xlsx(here::here("data", "indicator_alcohol_consumption _20100830.xlsx"), sheet = 1) 
...
- Join this new data using `dplyr` with the `reprohealth` data you have been working with, and make some observations about the process and result. You may need:
    - This [ModernDive section on joins](https://moderndive.com/3-wrangling.html#joins)
