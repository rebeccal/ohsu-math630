---
title: "MATH 530/630"
subtitle: "Integrative Lab 3 - Comparing Means: Self Assessment"
...
```{r install_data, eval=FALSE, echo = FALSE}
...
For the two independent samples case like what we have for comparing fishermen to non-fishermen, the formula is:
...
mercury %>% 
  ...
  visualize(method = "theoretical") +
            shade_p_value(tcrits,
            direction="both") +
  geom_vline(aes(xintercept = obs_t), color = "dodgerblue")
```
...
null_fish %>% 
  visualize() +
  shade_p_value(obs_stat = mercury_diff, direction = "two_sided")
...
The permutation test of the difference in means compares my observed difference in means, `r mercury_diff %>% pull() %>% round(2)`, to the permuted null distribution of the differences in means based on 1000 replicates (sampling without replacement; i.e., shuffling the 0/1 values in the `fisherman` variable). The permutation p-value is `r pdiff %>% pull() %>% round(3)`, indicating that- given the random variation present in `total_mercury` values- a difference in means as or more extreme than `r mercury_diff %>% pull() %>% round(2)` would only be expected with `r pdiff %>% pull() * 100`% probability.
...
The permutation test of the t-statistic compares my observed t-statistic, `r obs_t %>% round(2)`, to the permuted null distribution of the t-statistic based on 1000 replicates (sampling without replacement; i.e., shuffling the 0/1 values in the `fisherman` variable). The permutation p-value is `r tdiff %>% pull() %>% round(2)`, indicating that- given the random variation present in `total_mercury` values- a t-statistic as or more extreme than `r obs_t %>% round(2)` would only be expected with `r tdiff %>% pull() * 100`% probability. This lines up relatively well with my results based on the theoretical t-distribution, which gave a p-value of `r 2*min(upper,lower) %>% round(3)`.
